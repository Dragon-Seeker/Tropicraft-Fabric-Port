...
public class HomeTreeBranchFeature<T extends HomeTreeBranchConfig> extends Feature<T> {
    ...
    public HomeTreeBranchFeature(Codec<T> codec) {
        super(codec);
        this.vinesFeature = new ConfiguredFeature<>(TropicraftFeatures.VINES, new RainforestVinesConfig(4, 13, 12));
    }

    @Override
    public boolean generate(WorldGenLevel world, ChunkGenerator generator, Random random, BlockPos pos, T config) {
