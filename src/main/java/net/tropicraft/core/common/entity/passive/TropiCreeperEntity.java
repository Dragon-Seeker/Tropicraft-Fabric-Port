...
public class TropiCreeperEntity extends PathfinderMob {
    ...
    public boolean causeFallDamage(float distance, float damageMultiplier, DamageSource damageSource) {
        boolean fall = super.causeFallDamage(distance, damageMultiplier, damageSource);
        ...
        if (!this.level.isClientSide) {
            ...
            for (int i = 0; i < 3 * radiusSq; i++) {
                ...
                if (attempt.distSqr(center) < radiusSq) {
                    ...
                    BlockState state = SMALL_FLOWERS[random.nextInt(SMALL_FLOWERS.length)].defaultBlockState(); //.Blocks.SMALL_FLOWERS.getRandom(random).getDefaultState();
